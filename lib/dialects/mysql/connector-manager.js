...
module.exports = (function() {
  var ConnectorManager = function(sequelize, config) {
    ...
    this.mysql =  this.config.native ? require('mysql-libmysqlclient') : require('mysql');
  ...
  var connect = function(done, config) {
    ...
    var connection = null;
    if (this.config.native) {
      connection = this.mysql.createConnectionSync(config.host, config.username, config.password, config.database, config.port)
    } else {
